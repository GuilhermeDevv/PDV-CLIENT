...
// Store imports
import { useStoreInfoUser, useStoreToken, useStoreUser } from '@/lib/store';

// Service imports
import {
  ListInfoUserServiceContract,
  ListUserServiceContract,
} from '@/services/menu-service';

// Type definitions
export type MenuServiceRegistry = {
  listUserService: ListUserServiceContract;
  listInfoUserService: ListInfoUserServiceContract;
};

export function useMenuModel(props: MenuServiceRegistry) {
  const { listUserService, listInfoUserService } = props;

  const token = window.localStorage.getItem('token');
  const { setUser } = useStoreUser(state => state);
  const { setInfoUser } = useStoreInfoUser(state => state);
  const { setToken } = useStoreToken(state => state);
  ...
  const { data: infoUser } = useQuery({
    queryKey: ['user-info'],
    queryFn: () => listInfoUserService.exec(),
    enabled: !!token,
    staleTime: 1000 * 60 * 5,
    gcTime: 1000 * 60 * 10,
  });
  ...
  useEffect(() => {
    ...
    if (infoUser) setInfoUser(infoUser);
  }, [user, infoUser]);

  const getActiveLink = (url: string) => url === pathname;

  const toogleMenuUser = useCallback(() => {
    setIsOpen(!isOpen);
  }, [isOpen]);
